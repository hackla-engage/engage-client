    ...
    return this.props.preferenceList.map((preference, index) => {
      ...
      return (
        <div key={index} className="panel panel-default">
          ...
          <div
            className={`panel-collapse collapse ${item}`}
            style={{ backgroundColor: 'white', color: '#18bc9c', borderRadius: '10px' }}
            role="tabpanel"
          >
            <div className="panel-body" style={{ paddingLeft: '25px', paddingRight: '25px' }}>
              <span>{preference.info}</span>
            </div>
          </div>
        </div>
      );
  ...
  render() {
    ...
    return (
      <div className="container">
        ...
        <hr />
        <div style={{ textAlign: 'center' }}>{this.selected()}</div>
      </div>
    );
  }
}
